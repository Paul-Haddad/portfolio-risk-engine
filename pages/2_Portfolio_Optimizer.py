...
if not run:
    st.info("Enter your assets in the sidebar, then run the optimiser.")
    st.stop()
if len(tickers) < 2:
...
t1, t2, t3 = st.tabs(["Efficient frontier", "Allocations", "Correlations"])
...
with t1:
    ...
    st.caption("Each dot is a random long-only mix; the curve is the best achievable return "
               "for each level of risk. The star is the highest-Sharpe portfolio, the diamond "
               "the lowest-risk one. Your equal-weight mix (x) sits inside the frontier — the "
               "gap to the curve is the improvement optimisation buys you.")
...
with t2:
    ...
    with c1:
        ...
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        comp = pd.DataFrame({"Max Sharpe": w_ms, "Min variance": w_mv,
                             "Equal weight": eq}, index=tickers)
...
with st.expander("Methodology — how the optimiser works"):
    st.markdown(
        ...
        - **Efficient frontier:** for every target return, the optimiser finds the long-only
          mix with the *lowest* variance (weights ≥ 0, summing to 100%). That traces the
        ...
        - **Caveat:** optimisation uses *past* returns as the estimate of the future, so treat
          it as a disciplined starting point, not a guarantee. It pairs naturally with the
